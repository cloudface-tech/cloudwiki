...
export async function task (payload) {
  ...
  try {
    const metadata = await fetch('https://github.com/gmowses/cloudwiki-locales/raw/main/locales/metadata.json').then(r => r.json())
    for (const lang of metadata.languages) {
      ...
      const strings = await fetch(`https://github.com/gmowses/cloudwiki-locales/raw/main/locales/${langFilename}.json`).then(r => r.json())
      if (strings) {
