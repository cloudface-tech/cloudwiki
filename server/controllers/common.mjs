...
export default function () {
  ...
  /**
   * Restore a site asset from DB to disk cache if missing
   */
  async function restoreSiteAssetFromDb (assetId, destPath) {
    const row = await WIKI.db.knex('assets').where('id', assetId).select('data', 'mimeType').first()
    if (row?.data) {
      await fs.mkdir(path.dirname(destPath), { recursive: true })
      await fs.writeFile(destPath, row.data)
      return true
    }
    return false
  }
    ...
    switch (req.params.resource) {
      case 'logo': {
        if (site.config.assets.logo) {
          const logoPath = path.join(siteAssetsPath, `logo-${site.id}.${site.config.assets.logoExt}`)
          try {
            await fs.access(logoPath)
          } catch {
            await restoreSiteAssetFromDb(site.config.assets.logo, logoPath)
          }
      ...
      case 'favicon': {
        if (site.config.assets.favicon) {
          const faviconPath = path.join(siteAssetsPath, `favicon-${site.id}.${site.config.assets.faviconExt}`)
          try {
            await fs.access(faviconPath)
          } catch {
            await restoreSiteAssetFromDb(site.config.assets.favicon, faviconPath)
          }
      ...
      case 'loginbg': {
        if (site.config.assets.loginBg) {
          const loginBgPath = path.join(siteAssetsPath, `loginbg-${site.id}.jpg`)
          try {
            await fs.access(loginBgPath)
          } catch {
            await restoreSiteAssetFromDb(site.config.assets.loginBg, loginBgPath)
          }
          res.sendFile(loginBgPath)
        } else {
