              ...
              q-item
                ...
                q-item-section(side)
                  q-btn.acrylic-btn(
                    ...
                    icon='las la-arrow-circle-right'
                    color='primary'
                    @click='executeAction(state.selectedTarget, idx)'
...
async function executeAction (targetKey, handler) {
  $q.loading.show({ message: t('admin.storage.actionRunning') })
  try {
    const resp = await APOLLO_CLIENT.mutate({
      mutation: gql`
        mutation ($targetId: UUID!, $handler: String!) {
          executeStorageAction(targetId: $targetId, handler: $handler) {
            operation {
              succeeded
              message
            }
    ...
    const op = resp?.data?.executeStorageAction?.operation
    if (op?.succeeded) {
      $q.notify({
        type: 'positive',
        message: op.message || 'Action completed successfully.'
      })
    } else {
      $q.notify({
        type: 'negative',
        message: op?.message || 'Action failed.',
        timeout: 10000
      })
    }
  } catch (err) {
...
async function setupGitHub () {
  ...
  state.setupCfg.manifest = JSON.stringify({
    name: `CloudWiki - ${adminStore.currentSiteId.slice(-12)}`,
    description: 'Connects your CloudWiki to GitHub repositories and synchronize their contents.',
...
function generateGraph () {
  ...
  state.deliveryNodes = {
    ...
    pages_wiki: { name: 'CloudWiki', icon: '/_assets/logo-cloudwiki.svg', color: '#161b22' }
  }
  state.deliveryEdges = {
  ...
  for (const [i, tp] of types.entries()) {
    ...
    if (dt) {
      ...
      state.deliveryNodes[`${tp.key}_wiki`] = { name: 'CloudWiki', icon: '/_assets/logo-cloudwiki.svg', color: '#161b22' }
